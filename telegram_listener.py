"""
Botón de pánico vía Telegram: polling liviano de getUpdates (urllib, sin requests).

  IA_TELEGRAM_PANIC_ENABLE=1      — solo entonces `poll_telegram_panic_commands` actúa.
  Los comandos se aceptan SOLO desde TELEGRAM_CHAT_ID (autorización).

Comandos (primer token del mensaje, mayúsculas ignoradas):
  ...
  IA_TELEGRAM_PANIC_OFFSET_FILE — JSON con último update_id procesado (default ia_telegram_panic_offset.json).
  IA_TELEGRAM_PANIC_CONSUME_BACKLOG=0 — en el primer ciclo sólo avanza offset sin ejecutar comandos
    para no disparar STOP antiguos tras reinicio (default 1 = consumir comandos sí).

Opcional tras DETENER/STOP:
  IA_TELEGRAM_PANIC_CLOSE_POSITIONS=1 (default)
  IA_TELEGRAM_PANIC_CANCEL_PENDING=1
...
def _load_offset() -> int | None:
    """Siguiente update_id esperado por getUpdates (último procesado + 1)."""
    p = _offset_file()
    if not p.is_file():
        return None
    try:
...
def _get_updates(tok: str, offset: int | None) -> tuple[list[dict[str, Any]], list[int]]:
    """
    Llama API getUpdates; devuelve lista de objetos Update y todos los update_id recibidos.
    """
...
def cerrar_posiciones_panico_ia_auto(*, deviation: int | None = None) -> tuple[int, int]:
    """
    Cierra todas las posiciones con BOT_MAGIC. (ok_intentos, fallos_reportados).
    """
    try:
        ...
        print("[panic] MT5/mw5_prices no disponible.", file=sys.stderr)
        return 0, 1
